...
const ViewersPage = () => {
  ...
  const viewers = allMembers.filter((m) => m.type === "VIEWER");

  // Edit modal state (same as Members page)
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setEditError("Failed to update viewer. Try again.");
      console.error("Edit viewer error:", err);
    } finally {
      setEditLoading(false);
    }
  };
      ...
      <div className="max-w-full mx-auto">
        <div className="bg-grayDash py-3 px-4 rounded-[17px] shadow-xl">
          <div className="frLine flex justify-between items-center mb-3">
            <h1 className="text-gray2 text-[16px] font-radioCanada">Viewers</h1>
          ...
          {isLoading ? (
            <div className="text-white py-8 text-center">Loading viewers...</div>
          ) : error ? (
            <div className="text-red-400 py-4">Failed to load viewers</div>
          ) : viewers.length === 0 ? (
            <div className="text-gray2 py-6 text-center">No viewers found</div>
          ) : (
              ...
              {viewers.map((member) => (
                ...
                >
                  <img
                    ...
                    alt={member.user?.firstName || member.user?.email || "Viewer"}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-xs text-pink2 font-medium mr-2">
                      {member.type || (member.member?.role || "").toUpperCase() || "VIEWER"}
                    </div>
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[#1E1E1E] rounded-2xl w-full max-w-md p-6 z-10">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-white font-semibold">Edit Viewer</h2>
              <button onClick={closeEditModal} className="text-gray2">✕</button>
            </div>
